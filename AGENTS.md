...
* Demandas de implementação exigem classificação explícita do usuário como `bug`, `melhoria`, `evolução` ou `nova funcionalidade`.
...
* Se o usuário passar a pedir alteração, correção ou entrega, a demanda deve ser classificada ou reclassificada para `bug`, `melhoria`, `evolução` ou `nova funcionalidade`.
...
Para `bug`, `melhoria`, `evolução` e `nova funcionalidade`, antes de alterar qualquer arquivo de implementação, o agente deve:
...
1. `README.md` se existir
2. `documentacao/project-context.md`
3. outros arquivos relevantes dentro de `documentacao/`, quando existirem
4. `src/` e os arquivos diretamente envolvidos na demanda, quando existirem
...
* `documentacao/project-context.md` deve consolidar visão do produto, decisões do MVP, arquitetura vigente e etapa atual de implementação.
* `documentacao/` concentra documentação funcional, de produto, de uso e de apoio à execução.
* `src/` é a fonte de verdade da implementação atual quando existir no repositório.
...
* **Skills carregadas na sessão:** usar as skills disponibilizadas pelo ambiente atual do Codex
* **Documentação canônica local:** `/workspace/documentacao`

**Importante:** no Codex, `workflow` não é entidade nativa executável.
A execução deve ocorrer por **skills orquestradoras** (`workflow-*`) quando elas estiverem disponíveis no ambiente atual.

## 🏗️ Direção Atual do Projeto

Decisões vigentes para o MVP:

* arquitetura de aplicação: **monólito modular em Next.js**
* foco inicial: **web app**
* persistência prioritária: **MongoDB**
* colaboração multiusuário: **fora do MVP atual**
* contexto oficial do projeto e da fase corrente: `documentacao/project-context.md`
